...
- Added multilingual consultant support in commit `ccffced`: EN/ID/KO translations cover greetings, controls, placeholders, errors, source labels, and unknown-data notices; language selectors, website-language changes, explicit language requests, mid-conversation switching, and multilingual search-term mapping are supported. The build for `ccffced` completed successfully on GitHub Pages.
